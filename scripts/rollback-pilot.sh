...
[[ "$registry" == *.dkr.ecr.*.amazonaws.com ]] || { echo "Previous release registry is not an AWS ECR registry" >&2; exit 2; }
login_region="${AWS_REGION:-${AWS_DEFAULT_REGION:-mx-central-1}}"
aws ecr get-login-password --region "$login_region" | docker login --username AWS --password-stdin "$registry"
