...
class PublicListingSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = Listing
        fields = ["id", "slug", "title", "propertyType", "propertyTypeName", "sourceType", "status", "published", "featured", "price", "priceMax", "currency", "priceLabel", "state", "municipality", "neighborhood", "latitude", "longitude", "bedrooms", "bathrooms", "halfBathrooms", "parkingSpaces", "constructionM2", "constructionAreaBasis", "landM2", "landAreaBasis", "gardenM2", "shortDescription", "description", "amenities", "developerId", "developerName", "developmentId", "developmentName", "modelName", "heroImage", "gallery", "createdAt", "updatedAt"]

    def current_price(self, obj):
        return next((x for x in obj.prices_cache if x.effective_to is None), None) if hasattr(obj, "prices_cache") else obj.offering.prices.filter(effective_to__isnull=True).first()
    ...
    def get_amenities(self, obj): return list(OfferingAmenity.objects.filter(offering=obj.offering).values_list("amenity__name", flat=True))
    ...
    def media(self, obj): return list(obj.media_links.select_related("media").order_by("sort_order"))
    ...
    def get_status(self, obj):
        current = obj.offering.availability_history.filter(effective_to__isnull=True).first(); return current.status.lower() if current else None
...
class AdminListingSerializer(serializers.ModelSerializer):
    ...
    def get_current_price(self, obj):
        p = obj.offering.prices.filter(effective_to__isnull=True).first(); return PriceRecordSerializer(p).data if p else None
    def get_current_availability(self, obj):
        a = obj.offering.availability_history.filter(effective_to__isnull=True).first(); return AvailabilityRecordSerializer(a).data if a else None
