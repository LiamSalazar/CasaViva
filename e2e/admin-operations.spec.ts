...
test("Founder administra campaña y gasto sin borrar historia", async ({ page }) => {
  ...
  await page.getByLabel("Canal").fill("paid_social");
  ...
  const spendRow = page.getByRole("row").filter({ hasText: campaign }).filter({ hasText: "$3,000" });
  ...
  await expect(page.getByRole("row").filter({ hasText: campaign }).filter({ hasText: "$3,000" })).toContainText("Anulado");
