...
import { serverApi, serverFetchAllPages } from "@/services/server-api";
...
export default async function sitemap(): Promise<MetadataRoute.Sitemap> {
  ...
  const [listings, developments, guides] = await Promise.all([
    serverFetchAllPages<{ slug: string; updatedAt: string }>("/api/v1/public/listings/?page_size=100"),
    serverApi<{ results: Array<{ slug: string; updated_at?: string }> }>("/api/v1/public/developments/"),
    serverFetchAllPages<{ slug: string; createdAt: string }>("/api/v1/public/guides/?page_size=100"),
  ]);
  return [
    ...
    ...listings.map((x) => ({ url: `${origin}/propiedades/${x.slug}`, lastModified: x.updatedAt, changeFrequency: "weekly" as const, priority: 0.8 })),
    ...
    ...guides.map((x) => ({ url: `${origin}/guias/${x.slug}`, lastModified: x.createdAt, changeFrequency: "monthly" as const, priority: 0.6 })),
  ];
}
